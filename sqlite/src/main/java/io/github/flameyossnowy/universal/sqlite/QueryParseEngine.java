...
public class QueryParseEngine {
    ...
    public @NotNull String parseRepository(@NotNull RepositoryMetadata.RepositoryInformation metadata, SQLiteRepositoryAdapter<?> repository) {
    ...
    private static void generateColumns(@NotNull RepositoryMetadata.RepositoryInformation metadata,
                                        StringJoiner joiner,
                                        SQLiteRepositoryAdapter<?> repository,
    ...
    private static @NotNull Result generateColumn(@NotNull RepositoryMetadata.FieldData<?> data,
        ...
        if (data.autoIncrement()) {
            fieldBuilder.append(" AUTOINCREMENT");
        }
    ...
    private static <T> String resolveType(@NotNull RepositoryMetadata.FieldData<T> data,
        ...
        SQLiteValueTypeResolver<T> resolver = (SQLiteValueTypeResolver<T>) registry.getResolver(data.type());
    ...
    private static <T> @Nullable SQLiteValueTypeResolver<T> createResolver(final RepositoryMetadata.@NotNull FieldData<T> data, final @NotNull SQLiteRepositoryAdapter<?> repository, final ValueTypeResolverRegistry registry) {
        SQLiteValueTypeResolver<T> resolver;
        if (Enum.class.isAssignableFrom(data.type())) {
            ...
            resolver = (SQLiteValueTypeResolver<T>) registry.getResolver(enumClass);
        } else {
    ...
    private static <T> @Nullable SQLiteValueTypeResolver<T> parseResolver(final RepositoryMetadata.@NotNull FieldData<T> data,
                                                                          final @NotNull SQLiteRepositoryAdapter<?> repository) {
